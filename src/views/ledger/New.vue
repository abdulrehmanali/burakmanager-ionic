        ...
        <ion-card>
          <ion-card-header>
            <ion-row class="ion-align-items-start">
              <ion-col>
                <ion-card-title style="margin: 10px 0px">Product's (Optional)</ion-card-title>
              </ion-col>
              <ion-col>
                    ...
                    <ion-col>
                      <ion-item>
                        <ion-input
                          ...
                          @keyup="product.selectedQuantity = ($event.target.value >= product.stockQuantity?product.stockQuantity:$event.target.value)"
                    ...
                    <ion-col>
                      <ion-item>
                        <ion-input
                          ...
                          @keyup="product.price = $event.target.value"
                          :value="product.price"
                        ></ion-input>
                      </ion-item>
                    </ion-col>
        ...
        <ion-card>
          ...
          <ion-list>
            <ion-item>
              ...
              <ion-label slot="end">
                <span style="margin:10px 10px 0 0;">{{customer.name}}</span>
                <ion-button @click="openSelectCustomerModal" class="ion-float-end">{{(customer.name?'Edit':'Select')}} Customer</ion-button>
              </ion-label>
            </ion-item>
            <ion-item>
              ...
              <ion-select
                ...
                cancel-text="Dismiss"
              >
                <ion-select-option value="cash" :selected="paymentMethod == 'cash'"
                  >Cash</ion-select-option
                >
                <ion-select-option value="cheque" :selected="paymentMethod == 'cheque'"
              ...
              <ion-item v-if="paymentMethod == 'cheque' || paymentMethod == 'bankTransfer'">
                <ion-label>Bank Name: </ion-label>
                <ion-input type="text" @keyup="bankName = $event.target.value" :value="bankName" ></ion-input>
              </ion-item>
              <ion-item v-if="paymentMethod == 'cheque'">
                <ion-label>Cheque Number: </ion-label>
                <ion-input type="text" @keyup="chequeNumber = $event.target.value" :value="chequeNumber" ></ion-input>
              </ion-item>
              <ion-item v-if="paymentMethod == 'bankTransfer'">
                <ion-label>Transaction Id: </ion-label>
                <ion-input type="text" @keyup="transactionId = $event.target.value" :value="transactionId" ></ion-input>
              </ion-item>
              <ion-item v-if="selectedProducts.length">
              ...
              <ion-item>
                ...
                <ion-input type="number" @keyup="amountReceived = ($event.target.value>=total?amountReceived:$event.target.value)" :value="amountReceived" :maxlength="total"></ion-input>
              ...
              <ion-select
                ...
                cancel-text="Dismiss"
              >
                <ion-select-option value="received" :selected="paymentStatus == 'received'"
                  >Received</ion-select-option
                >
                <ion-select-option value="pending" :selected="paymentStatus == 'pending'"
                  >Pending</ion-select-option
                >
                <ion-select-option value="on-hold" :selected="paymentStatus == 'on-hold'"
            ...
            <ion-item>
                ...
                <ion-textarea @keyup="note = $event.target.value" :value="note"></ion-textarea>
              </ion-item>
              <ion-item>
...
export default {
  ...
  setup() {
    ...
    const getTotal = () =>{
      state.total = state.selectedProducts.map((product: any)=>{return (product.price * product.selectedQuantity)}).reduce((a: any, b: any) => a + b, 0);
      return state.total;
    }
    return {
      ...toRefs(state),
